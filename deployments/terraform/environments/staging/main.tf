...
data "terraform_remote_state" "metadata" {
  ...
  config = {
    organization = "pecdevelopers"
    workspaces = {
      name = "pec-events-app-metadata-staging"
    }
  }
}
...
module "backend" {
  ...
  domain_name     = local.frontend_domain # Backend resolves to api.staging.events.pecdevelopers.com
...
module "frontend" {
  ...
  domain_name     = local.frontend_domain # Frontend resolves to staging.events.pecdevelopers.com
  dns_zone_name   = var.dns_zone_name
  application_tag = local.application_tag
}
